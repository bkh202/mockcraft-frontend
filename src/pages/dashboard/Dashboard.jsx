...
const StatCard = ({ label, value, icon }) => (
  <div className="p-4 rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow flex items-center gap-3">
...
const Dashboard = () => {
  ...
  if (!dashboard) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="text-center max-w-sm p-6 bg-white rounded-3xl shadow-lg border border-slate-100">
          <div className="w-14 h-14 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h3 className="text-lg font-black text-slate-900 mb-2">Unable to load dashboard</h3>
          <p className="text-slate-500 mb-5 text-sm font-medium">Please try refreshing the page.</p>
          <button onClick={() => window.location.reload()}
            className="w-full px-4 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors text-sm">
            Refresh Page
          </button>
        </div>
      </div>
  ...
  const freeModules = [
    { title: "Aptitude Tests", count: "Quantitative & Logic", color: "from-slate-700 to-slate-800", link: "/aptitude" },
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="p-4 sm:p-6 lg:p-8 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-linear-to-r from-white to-blue-50/50">
            ...
            <div className="flex items-center gap-3 bg-white p-3 sm:p-4 rounded-2xl shadow-sm border border-slate-100 self-start sm:self-auto shrink-0">
              ...
              <div>
                ...
                <p className="text-lg font-black text-slate-800">3 Days</p>
              </div>
            </div>
          </div>

          {/* Stats Grid — 2 cols mobile, 4 cols desktop */}
          <div className="p-4 sm:p-6 lg:p-8 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 bg-slate-50/30">
        ...
        <div className="bg-linear-to-br from-indigo-900 to-slate-900 rounded-2xl sm:rounded-3xl shadow-lg p-0.5 relative overflow-hidden">
          ...
          <div className="bg-slate-900/80 backdrop-blur-md rounded-[20px] sm:rounded-[22px] p-4 sm:p-6 lg:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10 border border-white/10">
            <div className="min-w-0 flex-1">
              <h2 className="text-lg sm:text-xl lg:text-2xl font-black text-white mb-2 flex items-center gap-2">
                <span>🧠</span> AI Learning Path
              </h2>
              <p className="text-indigo-200 font-medium mb-3 text-sm">
                Based on your last attempt, targeted practice is the fastest way to improve.
              </p>
              {dashboard?.weakTopics?.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  <span className="text-xs font-bold text-slate-400 py-1.5 mr-1">Focus:</span>
                  {dashboard.weakTopics.slice(0, 3).map((topic, i) => (
                    <span key={i} className="px-3 py-1 bg-red-500/20 border border-red-500/30 text-red-300 rounded-lg text-xs font-semibold">{topic}</span>
                  ))}
                </div>
              ) : (
                <span className="px-3 py-1 bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 rounded-lg text-xs font-semibold">
            ...
            <Link to="/premium/engineering/recommended-quiz" className="shrink-0 w-full sm:w-auto">
              <button className="w-full sm:w-auto px-5 py-3 bg-white text-indigo-900 font-black rounded-xl hover:shadow-lg hover:scale-105 transition-all text-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          {/* Free Modules — full width mobile, 2/3 desktop */}
          <div className="lg:col-span-2 bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-slate-100">
              <h2 className="text-lg font-black text-slate-900 mb-0.5">🎯 Practice Banks</h2>
              <p className="text-slate-500 text-xs font-medium">Free AI-powered question banks</p>
            </div>
          ...
          {/* Recent Activity — full width mobile, 1/3 desktop */}
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-slate-100 flex justify-between items-center">
              ...
              <Link to="/history" className="text-xs font-bold text-blue-600 hover:text-blue-800">View All</Link>
            </div>
            <div className="p-4 sm:p-5 bg-slate-50/50">
              {dashboard?.recentAttempts?.length > 0 ? (
                <div className="space-y-3">
                  {dashboard.recentAttempts.slice(0, 4).map((attempt, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${attempt.correct ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                          {attempt.correct ? (
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-slate-900 text-xs truncate">{attempt.subject}</p>
                          <p className="text-xs font-medium text-slate-400 truncate">{attempt.topic}</p>
                        </div>
                      </div>
                      <div className="text-right shrink-0 ml-2">
                        <p className="font-black text-slate-900 text-sm">{attempt.accuracy}%</p>
                        <p className="text-xs font-bold text-slate-400 uppercase">{attempt.mode}</p>
                      </div>
                    </div>
                  ))}
                </div>
