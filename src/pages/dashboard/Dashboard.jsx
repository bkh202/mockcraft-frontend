...
const StatCard = ({ label, value, icon }) => (
  <div className="p-5 rounded-2xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow flex items-center gap-4">
    <div className="w-12 h-12 rounded-xl bg-gray-100 text-black flex items-center justify-center text-2xl shrink-0 border border-gray-200">
...
const CategoryCard = ({ title, count, link, premium = false }) => (
  <Link
    to={link}
    className={`group block p-6 rounded-2xl bg-white border border-gray-200 hover:border-gray-400 transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 relative overflow-hidden`}
  >
    ...
    <div className="relative z-10">
      <h3 className="text-xl font-extrabold text-black mb-2 leading-tight break-words">{title}</h3>
      <p className="text-base text-gray-600 font-medium break-words">{count}</p>
      {premium && (
...
const UpgradePopup = ({ feature, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[99999] p-4">
      <div className="bg-white border border-gray-200 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
  ...
  if (!dashboard) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white p-4">
        <div className="text-center max-w-sm p-8 bg-white border border-gray-200 rounded-3xl shadow-sm">
    ...
    <div className="min-h-screen bg-white font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10 space-y-6 sm:space-y-8">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 sm:p-8 lg:p-10 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="min-w-0">
              <div className="inline-flex items-center gap-3 px-4 py-2 bg-gray-100 text-gray-700 rounded-full text-sm font-bold uppercase tracking-wider mb-3 border border-gray-200">
                <span className="w-2 h-2 rounded-full bg-black animate-pulse"></span>
                Student Dashboard
              </div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-black tracking-tight truncate">
                Welcome,{" "}
              ...
              <p className="text-gray-600 font-light mt-2 text-lg">
                Here is your learning trajectory for today.
              </p>
            </div>

            <div className="flex items-center gap-4 bg-white p-4 sm:p-5 rounded-2xl shadow-sm border border-gray-200 self-start sm:self-auto shrink-0">
              <div className="w-12 h-12 bg-gray-100 flex items-center justify-center rounded-xl text-black border border-gray-200 text-2xl">
                <i className="fa fa-fire"></i>
              </div>
              <div>
                <p className="text-sm font-bold text-gray-600 uppercase tracking-wider">Streak</p>
                <p className="text-2xl font-black text-black">3 Days</p>
              </div>
            </div>
          </div>

          {/* Stats */}
          <div className="p-6 sm:p-8 lg:p-10 grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 sm:p-8 lg:p-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div className="min-w-0 flex-1">
              <h2 className="text-2xl sm:text-3xl font-black text-black mb-3 flex items-center gap-3">
              ...
              {dashboard?.weakTopics?.length > 0 ? (
                <div className="flex flex-wrap gap-3">
                  <span className="text-base font-bold text-gray-700 py-1.5 mr-1">Focus:</span>
                  {dashboard.weakTopics.slice(0, 3).map((topic, i) => (
                    <span
                      key={i}
                      className="px-4 py-2 bg-gray-100 border border-gray-200 text-black rounded-lg text-sm font-semibold break-words"
                    >
                      {topic}
                    </span>
                  ))}
                ...
                <span className="px-4 py-2 bg-gray-100 border border-gray-200 text-black rounded-lg text-sm font-semibold">
            ...
            <Link to="/premium/engineering/recommended-quiz" className="shrink-0 w-full sm:w-auto">
              <button className="w-full sm:w-auto px-6 py-4 bg-black text-white font-black rounded-xl hover:bg-gray-800 transition text-base shadow-sm">
        ...
        {/* ─── 3. PREMIUM MODULES ─── */}
        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-6 sm:p-8 lg:p-10 border-b border-gray-200 flex flex-col xs:flex-row xs:items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-black text-black mb-1 flex items-center gap-3">
            ...
            {!isPremium() && (
              <span className="inline-flex items-center px-4 py-2 bg-black text-white rounded-xl text-sm font-black shadow-sm self-start xs:self-auto shrink-0 border border-gray-300">
                UPGRADE REQUIRED
              </span>
            )}
          </div>
                ...
                {!isPremium() && (
                  <>
                    ...
                    <div className="absolute inset-0 bg-white/60 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center pointer-events-none">
                      <span className="bg-black text-white text-sm font-black px-4 py-2 rounded-full shadow-lg pointer-events-auto">
                        Unlock Access
                      </span>
                    </div>
                  </>
          ...
          <div className="lg:col-span-2 bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 sm:p-8 border-b border-gray-200">
              <h2 className="text-2xl sm:text-3xl font-black text-black mb-1">
                <i className="fa fa-bullseye text-black mr-3"></i> Practice Banks
              </h2>
              <p className="text-gray-600 text-lg font-light">Free AI-powered question banks</p>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 sm:p-8 border-b border-gray-200 flex justify-between items-center">
              <h2 className="text-2xl sm:text-3xl font-black text-black">
                <i className="fa fa-clock text-black mr-3"></i> Activity
              </h2>
              <Link to="/history" className="text-base font-bold text-gray-600 hover:text-black transition">
                View All
              </Link>
            </div>
                  ...
                  {dashboard.recentAttempts.slice(0, 4).map((attempt, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-black text-base truncate">{attempt.subject}</p>
                          <p className="text-sm font-medium text-gray-600 truncate">{attempt.topic}</p>
                        </div>
                      </div>
                      <div className="text-right shrink-0 ml-3">
                        <p className="font-black text-black text-xl">{attempt.accuracy}%</p>
