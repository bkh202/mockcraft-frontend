...
export default function InterviewSetup() {
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6">
      <div className="max-w-xl w-full bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">

        {/* Header */}
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center border border-gray-200">
            <i className="fa fa-sliders-h text-xl text-black"></i>
          </div>
          <div>
            <h2 className="text-2xl font-extrabold text-black">Configure Interview</h2>
            <p className="text-sm text-gray-600">Set up your AI mock interview session</p>
          </div>
        </div>

        {/* Error */}
        {error && (
          <div className="flex items-start gap-2 bg-red-50 text-red-700 p-3 rounded-xl mb-5 text-base font-semibold border border-red-200">
            <i className="fa fa-exclamation-circle text-lg mt-0.5 shrink-0"></i> {error}
          </div>
        )}
        ...
        <div className="flex gap-3 mb-6">
          <button type="button" onClick={() => setUseResumeMode(true)} disabled={!storedResume}
            ...
            className={`flex-1 py-3 rounded-xl font-bold border text-base transition ${
              useResumeMode ? 'bg-black border-black text-white' : 'bg-white border-gray-300 text-gray-600 hover:border-black hover:text-black'
            } disabled:opacity-40 disabled:cursor-not-allowed`}>
            <i className="fa fa-file-alt mr-1.5"></i> Use Parsed Resume
          </button>
          <button type="button" onClick={() => setUseResumeMode(false)}
            className={`flex-1 py-3 rounded-xl font-bold border text-base transition ${
              !useResumeMode ? 'bg-black border-black text-white' : 'bg-white border-gray-300 text-gray-600 hover:border-black hover:text-black'
            }`}>
            <i className="fa fa-pen mr-1.5"></i> Manual Entry
          </button>
        </div>
        ...
        <form onSubmit={startInterview} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1.5 uppercase tracking-wide">Target Company</label>
              <input type="text" value={formData.companyName} onChange={update('companyName')} required placeholder="e.g. Google"
                className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-black text-base focus:ring-2 focus:ring-black focus:border-black outline-none transition" />
            </div>
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1.5 uppercase tracking-wide">Job Role</label>
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-1.5 uppercase tracking-wide">Experience Level</label>
          ...
          {useResumeMode ? (
            <div className="flex items-start gap-3 bg-green-50 border border-green-200 p-4 rounded-xl text-base font-medium">
              <i className="fa fa-check-circle text-xl shrink-0 mt-0.5 text-green-700"></i>
              <div className="text-green-800">
                <span>Using skills and experience from your selected resume.</span>
                {storedResume?.name && <span className="block text-green-700 text-sm mt-1 font-bold">Candidate: {storedResume.name}</span>}
                {storedResume?.skills && (
                  <span className="block text-green-700/80 text-sm mt-1 line-clamp-2">
            ...
            <div>
              ...
              <textarea value={formData.resumeSkills} onChange={update('resumeSkills')} required rows={2}
                ...
                className="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-black text-base focus:ring-2 focus:ring-black focus:border-black outline-none transition resize-none" />
            </div>
          )}

          <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1.5 uppercase tracking-wide">Difficulty</label>
              <select value={formData.difficulty} onChange={update('difficulty')}
                className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-black text-base focus:ring-2 focus:ring-black focus:border-black outline-none transition">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1.5 uppercase tracking-wide">Questions</label>
              <input type="number" min="1" max="10" value={formData.questionCount} onChange={update('questionCount')} required
                className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-black text-base focus:ring-2 focus:ring-black focus:border-black outline-none transition" />
              <p className="text-xs text-gray-500 mt-1">Max 10 questions.</p>
            </div>
          </div>

          <button type="submit" disabled={loading}
            className="w-full bg-black hover:bg-gray-800 text-white font-bold py-4 rounded-xl transition flex justify-center items-center gap-2 disabled:opacity-50 shadow-sm text-lg border border-gray-300">
            {loading ? <><i className="fa fa-spinner fa-spin text-lg"></i> Generating Questions...</> : 'Generate Interview Session →'}
          </button>
